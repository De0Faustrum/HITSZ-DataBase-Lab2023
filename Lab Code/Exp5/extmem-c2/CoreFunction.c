...
void linearSearch() {
    ...
    while(TRUE) {           //开始读磁盘中的Block
        if ((bufferedReader = readBlockFromDisk(bufferedReaderAddress, &buf)) == NULL) {
            perror("Reading Block Failed!\n");
            return;
        }
        printf("读入数据块%d\n", bufferedReaderAddress);
        for (int i=0; i<TUPLES_PER_BLOCK; i++) {
            Tuple bufferedTuple;    //从读缓冲区获得元组
    ...
    if (foundCount) {   //将写缓冲区内的剩余数据写入磁盘block
        memcpy(bufferedWriter+foundCount*TUPLIE_LENGTH, emptyString, (TUPLES_PER_BLOCK-foundCount)*TUPLIE_LENGTH);  //反向清空写缓冲区的多余数据
        itoa(bufferedWriterAddress+writeCount+1, bufferedWriter+TUPLES_PER_BLOCK*TUPLIE_LENGTH, 10);    //将下个block的地址添加到写缓冲区
...
void mergeSort(int mode) {
    ...
    while (TRUE) {
        bufferedWriter = getNewBlockInBuffer(&buf);
        remnantBlock = termReadAddress-readAdrress;
        Tuple tempTuple;
        if(++remnantBlock >= 6) {
            remnantBlock = 6;       //临界情况
        }
        for(int i=0; i<remnantBlock; i++) {     //读入block并进行内部排序
            ...
            for(int j=0; j<TUPLES_PER_BLOCK; j++) {       //内排序
                temp = bufferedTuple[j].x;
                order = j;
                ...
                itoa(bufferedTuple[j].x, tempConvert, 10);      //内排序后写回block~DEBUG
        ...
        while (!isMerged(indexVector, remnantBlock)) {
            ...
            for (int i = 0; i < remnantBlock; i++) {
                if (indexVector[i] < TUPLES_PER_BLOCK) {
                    ...
                    element[i].x = UPPER_LIMIT;     //此路已归并完毕
                }
            }
            temp = UPPER_LIMIT;                  //选择6路中的极小值并入总集
            for(int i=0; i<remnantBlock; i++) {
                if (element[i].x < temp) {
                    temp = element[i].x;
                    order = i;
            ...
            itoa(element[order].x, tempConvert, 10);
            ...
            if(indexVector[order] < TUPLES_PER_BLOCK){
                indexVector[order]++;
            }
        }
        ...
        if (readAdrress == termReadAddress + 1) {
            break;
        }
    }
