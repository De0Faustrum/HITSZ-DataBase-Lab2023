//
// Created by De0Faustrum on 2023/11/8.
//
# include <stdio.h>
# include <stdlib.h>
# include <string.h>
...
int TwoPhaseMultiwayMergeSort(int start_blk_addr, int end_blk_addr, int write_base_addr)
{
    ...
    while (1)
    {
        ...
        for (int i = 0; i < num; i++)
        {
            ...
            {
                printf("IN LINEAR, ADDR = %d",rd_blk_ptr);
                perror("Reading Block Failed!\n");
                return -1;
            }
        }
    ...
    for (int i = 0; i < group_num; i++)
    {
        ...
        {
            printf("IN LINEAR, ADDR = %d",write_base_addr+i*6);
            perror("Reading Block Failed!\n");
            return -1;
        }
    }
